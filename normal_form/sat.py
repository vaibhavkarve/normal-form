...
from graphsat.cnf import (FALSE_CNF, TRUE_CNF, Assignment, Bool, Clause, Cnf,
                          Lit, Variable, absolute_value, assign, clause, cnf,
                          int_repr, lit, lits, neg, tauto_reduce, variable)
from graphsat.mhgraph import (HEdge, MHGraph, degree, graph_union, link,
                              mhgraph, pick_min_degree_vertex, sphr, vertices)
...
def cnf_pysat_satcheck(cnf_instance: Cnf) -> bool:
    ...
    try:
        if (result := Minisat22(int_repr(cnf_instance)).solve()) is None:
            raise RuntimeError("Minisat22 returned None as result.")
        return result
    except (TypeError, RuntimeError) as exc:
        # The Cnf was probably not in reduced form. Reduce and try again
        cnf_reduced: Cnf = tauto_reduce(cnf_instance)
        if cnf_reduced == cnf_instance:
            raise RuntimeError("Irreducible Cnf not getting solved by Minisat22") from exc
